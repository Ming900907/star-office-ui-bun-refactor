...
function hasBunBackend(root) {
  return fs.existsSync(path.join(root, "server", "index.ts"));
}
...
function findProjectRoot() {
  if (process.env.STAR_PROJECT_ROOT) {
    ...
    if (hasBunBackend(custom)) return custom;
  }

  const fromDir = __dirname;
  let cursor = fromDir;
  for (let i = 0; i < 8; i += 1) {
    if (hasBunBackend(cursor)) return cursor;
...
function spawnBackend(projectRoot) {
  const script = path.join(projectRoot, "server", "index.ts");
  if (!fs.existsSync(script)) {
    console.warn(`server/index.ts not found: ${script}`);
    return null;
  }

  const candidates = [];
  if (process.env.STAR_BACKEND_BUN) candidates.push(process.env.STAR_BACKEND_BUN);
  if (process.env.BUN_BIN) candidates.push(process.env.BUN_BIN);
  candidates.push(path.join(process.env.HOME || "", ".bun", "bin", "bun"));
  candidates.push("bun");

  for (const bin of candidates) {
    try {
      const child = spawn(bin, ["run", "server/index.ts"], {
        cwd: projectRoot,
        stdio: "inherit",
        env: {
          ...process.env,
          PORT: process.env.PORT || String(BACKEND_PORT),
        },
      });
...
function createWindows(projectRoot) {
  ...
  const miniPath = path.join(projectRoot, "desktop-pet", "src", "minimized.html");
  if (fs.existsSync(miniPath)) {
    miniWindow.loadFile(miniPath);
  } else {
    miniWindow.loadURL(`${BACKEND_BASE_URL}/electron-standalone?desktop=1&mini=1&v=${v}`);
  }
}
